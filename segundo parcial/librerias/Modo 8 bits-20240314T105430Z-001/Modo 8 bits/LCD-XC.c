...
#include "LCD-XC.h"
    ...
    void LCDInit()
    {
        ...
        LCD_POR_DIR=0x00; 		 // set port to out
        LCD_EN_DIR=0;
        LCD_RW_DIR=0;
        LCD_RS_DIR=0;
    ...
    *******************************************************************/
    void LCDWriteByte(unsigned char ch,unsigned char b)
    {
        //ch = ch >> 4;                   // always send the upper nibble
        ch = ch;                // mask off the nibble to be transmitted

        LCD_PORT = LCD_PORT & 0x00;     // clear the LCD_PORT
...
* (the \93Company\94) for its PICmicro\AE Microcontroller is intended and
* supplied to you, the Company\92s customer, for use solely and
...
* license.
*
* THIS SOFTWARE IS PROVIDED IN AN \93AS IS\94 CONDITION. NO WARRANTIES,
